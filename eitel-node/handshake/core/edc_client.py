...
class EDCClient:
    ...
    async def initiate_negotiation(
        ...
        Calls: POST /v3/contractnegotiations
        ...
        try:
            ...
            # Start with the policy from catalogue (if provided), otherwise build minimal policy
            policy_obj = dict(policy) if policy else {}
            # Ensure required fields are present
            if "@type" not in policy_obj:
                policy_obj["@type"] = "odrl:Offer"
            if "@id" not in policy_obj:
                policy_obj["@id"] = offer_id
            if "odrl:assigner" not in policy_obj:
                policy_obj["odrl:assigner"] = {"@id": counterparty_did}
            if "odrl:target" not in policy_obj:
                policy_obj["odrl:target"] = {"@id": asset_id}
